        ...
        <div class="mb-4 flex items-center justify-between sm:mb-6">
          <div class="flex items-center gap-2 sm:gap-3">
            ...
            <h3 class="text-lg font-bold text-gray-900 dark:text-gray-100 sm:text-xl">
              {{ isEdit ? '编辑 Copilot 账户' : '添加 Copilot 账户' }}
            </h3>
          </div>
          <button
          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              ...
              <input
                ...
                placeholder="例如：http://copilot-api:4141"
                required
                type="text"
              />
              <p v-if="errors.baseApi" class="mt-1 text-xs text-red-500">{{ errors.baseApi }}</p>
            </div>
            <div>
              <label class="mb-3 block text-sm font-semibold text-gray-700 dark:text-gray-300"
                >Access Key {{ isEdit ? '(留空不更新)' : '(可选)' }}</label
              >
              <input
                v-model="form.apiKey"
                class="form-input w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200"
                :class="{ 'border-red-500': errors.apiKey }"
                :placeholder="isEdit ? '留空表示不更新' : '可选；需要鉴权时填写'"
                :required="false"
                type="password"
              />
...
import { updateCopilotAccountApi, createCopilotAccountApi } from '@/utils/http_apis'
...
const validate = () => {
  ...
  if (!form.value.baseApi || form.value.baseApi.trim().length === 0) e.baseApi = 'Base API 不能为空'
...
const submit = async () => {
  ...
  try {
    if (isEdit.value) {
      ...
      const res = await updateCopilotAccountApi(props.account.id, updates)
      if (res.success) {
      ...
      const res = await createCopilotAccountApi(payload)
      if (res.success) {
        // 不在这里显示 toast，由父组件统一处理
        emit('success')
      } else {
